...
this IDbConnection cnn, string sql, object param = null, IDbTransaction transaction = null, int? commandTimeout = null, CommandType? commandType = null
        ...
        private static int ExecuteImpl(this IDbConnection cnn, ref CommandDefinition command)
        {
            ...
            if (multiExec != null)
            {
                ...
                {
                    string conStr = cnn == null ? "dbConnectionΪ��;" : string.IsNullOrEmpty(cnn.ConnectionString) == true ? "ConnectionStringΪ��;" : cnn.ConnectionString;
                    string errorMsg = string.Format("{0};{1};{2};{3};{4};{5}", command.CommandText, "", conStr.Substring(0, Regex.Matches(conStr, ";")[1].Index), ex.Message, ex.Source, ex.StackTrace);
                    DBLog.Process(RequestProfile.Username, RequestProfile.RequestKey, RequestProfile.ClientIP, "QueryImpl<T>", RequestProfile.MessageKey, "sql_exception", errorMsg, "JinRi.Notify");
                    throw ex;
                }
                finally
                {
...
this IDbConnection cnn, string sql, object param, IDbTransaction transaction, int? commandTimeout, CommandType? commandType
        ...
        private static IEnumerable<T> QueryImpl<T>(this IDbConnection cnn, CommandDefinition command, Type effectiveType)
        {
            ...
            try
            {
                ...
                {
                    string conStr = cnn == null ? "dbConnectionΪ��;" : string.IsNullOrEmpty(cnn.ConnectionString) == true ? "ConnectionStringΪ��;" : cnn.ConnectionString;
                    string errorMsg = string.Format("{0};{1};{2};{3};{4};{5}", command.CommandText, cmd.Parameters.GetParamKeyValues(), conStr.Substring(0, Regex.Matches(conStr, ";")[1].Index), ex.Message, ex.Source, ex.StackTrace);
                    DBLog.Process(RequestProfile.Username, RequestProfile.RequestKey, RequestProfile.ClientIP, "QueryImpl<T>", RequestProfile.MessageKey, "sql_exception", errorMsg, "JinRi.Notify");
                    throw ex;
                }
                wasClosed = false; // *if* the connection was closed and we got this far, then we now have a reader
...
#pragma warning restore 618
        ...
        private static int ExecuteCommand(IDbConnection cnn, ref CommandDefinition command, Action<IDbCommand, object> paramReader)
        {
            ...
            catch (Exception ex)
            {
                string conStr = cnn == null ? "dbConnectionΪ��;" : string.IsNullOrEmpty(cnn.ConnectionString) == true ? "ConnectionStringΪ��;" : cnn.ConnectionString;
                string errorMsg = string.Format("{0};{1};{2};{3};{4};{5}", command.CommandText, cmd.Parameters.GetParamKeyValues(), conStr.Substring(0, Regex.Matches(conStr, ";")[1].Index), ex.Message, ex.Source, ex.StackTrace);
                DBLog.Process(RequestProfile.Username, RequestProfile.RequestKey, RequestProfile.ClientIP, "ExecuteCommand", RequestProfile.MessageKey, "sql_exception", errorMsg, "JinRi.Notify");
                throw ex;
            }
            finally
            {
        ...
        private static T ExecuteScalarImpl<T>(IDbConnection cnn, ref CommandDefinition command)
        {
            ...
            catch (Exception ex)
            {
                string conStr = cnn == null ? "dbConnectionΪ��;" : string.IsNullOrEmpty(cnn.ConnectionString) == true ? "ConnectionStringΪ��;" : cnn.ConnectionString;
                string errorMsg = string.Format("{0};{1};{2};{3};{4};{5}", command.CommandText, cmd.Parameters.GetParamKeyValues(), conStr.Substring(0, Regex.Matches(conStr, ";")[1].Index), ex.Message, ex.Source, ex.StackTrace);
                DBLog.Process(RequestProfile.Username, RequestProfile.RequestKey, RequestProfile.ClientIP, "ExecuteScalarImpl<T>", RequestProfile.MessageKey, "sql_exception", errorMsg, "JinRi.Notify");
                throw ex;
            }
            finally
            {
        ...
        private static IDataReader ExecuteReaderImpl(IDbConnection cnn, ref CommandDefinition command, CommandBehavior commandBehavior, out IDbCommand cmd)
        {
            ...
            catch (Exception ex)
            {
                string conStr = cnn == null ? "dbConnectionΪ��;" : string.IsNullOrEmpty(cnn.ConnectionString) == true ? "ConnectionStringΪ��;" : cnn.ConnectionString;
                string errorMsg = string.Format("{0};{1};{2};{3};{4};{5}", command.CommandText, cmd.Parameters.GetParamKeyValues(), conStr.Substring(0, Regex.Matches(conStr, ";")[1].Index), ex.Message, ex.Source, ex.StackTrace);
                DBLog.Process(RequestProfile.Username, RequestProfile.RequestKey, RequestProfile.ClientIP, "ExecuteReaderImpl", RequestProfile.MessageKey, "sql_exception", errorMsg, "JinRi.Notify");
                throw ex;
            }
            finally
            {
