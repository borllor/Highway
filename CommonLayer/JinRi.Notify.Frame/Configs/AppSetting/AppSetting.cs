...
namespace JinRi.Notify.Frame
{
    public class AppSetting
    {
        ...
        public static string GetConfigFilename()
        {
            if (m_configFilename == null)
            {
                lock (m_configFilenameLockObj)
                {
                    if (m_configFilename == null)
                    {
                        m_configFilename = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "JinRi.Notify.Frame.config");
                        if (!File.Exists(m_configFilename))
                        {
                            m_configFilename = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Configs\\JinRi.Notify.Frame.config");
                        }
                        if (!File.Exists(m_configFilename))
                        {
                            m_configFilename = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Config\\JinRi.Notify.Frame.config");
                        }
                        if (!File.Exists(m_configFilename))
                        {
                            m_configFilename = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "ConfigFiles\\JinRi.Notify.Frame.config");
                        }
                        if (!File.Exists(m_configFilename))
                        {
                            m_configFilename = "";
                            Logger.Fatal("初始化配置文件JinRi.Notify.Frame.config时，未找到配置文件", new FileNotFoundException("未找到配置文件"));
                        }
                    }
                }
            }
        ...
        public static string InitDistributionConfig()
        {
            ...
            {
                Logger.Fatal("读取配置JinRi.Notify.Frame.config文件产生严重错误", ex);
            }
        ...
        public static string DefaultCacheProvider
        {
            get
            {
                ...
                return string.IsNullOrEmpty(val) ? "JinRi.Notify.Frame.WebCacheProvider, JinRi.Notify.Frame" : val;
            }
        }
